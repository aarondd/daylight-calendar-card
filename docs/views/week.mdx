...
<Note>
  Multi-day all-day events render as one continuous bar across the day columns they cover, with the title shown on the first visible day. When the columns wrap onto a second row on narrow screens, the bar repeats its title at the start of each row.
</Note>
